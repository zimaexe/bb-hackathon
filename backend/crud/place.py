...
class CRUDPlace(CRUDBase[Place, PlaceCreate, PlaceUpdate]):
    ...
    @staticmethod
    async def get_place_by_cordinates(
        db: AsyncSession, place_cordinates: str
    ) -> "Place":
        """
        ...
        result = await db.execute(
            select(Place).where(Place.place_cordinates == place_cordinates)
        )
        return result.scalar_one_or_none()

    @staticmethod
    async def add_fair_to_place(
        db: AsyncSession, place_cordinates: str, fair: "Fair"
    ) -> "Place":
        """
    ...
    @staticmethod
    async def remove_fair_from_place(
        db: AsyncSession, place_cordinates: str, fair: "Fair"
    ) -> "Place":
        """
